    ...
    const fetchData = async () => {
      try {
        ...
        const transactionsResponse = await fetch(
          `${VITE_API_URL}/api/transactions/last`,
          {
            headers: {
              Authorization: `Bearer ${user.token}`,
            },
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-6 p-6 max-w-7xl mx-auto">
        ...
        <meta
          name="description"
          content="View your financial overview — track income, expenses, savings, and budget performance in one place."
        />
        <meta
          name="keywords"
          content="financial dashboard, budget summary, expense overview, income tracking"
        />
        <meta
          name="author"
          content="Veihi Joy Tupai,  Cameron Pedro, _Rama Krishna Bhagi Perez, Bamutesiza Ronald"
        />

        <meta property="og:title" content="BudgetPlanner | Dashboard" />
        <meta
          property="og:description"
          content="Analyze your finances with an easy-to-read dashboard."
        />
        ...
        <meta
          ...
          content="Get a full view of your financial health in one place."
        />
        ...
        <div className="flex gap-4 items-center pt-6">
          ...
          <img
            className="max-w-[48px] animate-bounce"
            src="/money-wings.svg"
            alt=""
          />
        </div>
                  ...
                  >
                    ...
                    <div className="text-right w-1/2">
                      <p className="text-base font-medium text-green-500">
                        {currencySymbol}
                        {account.income_total.toFixed(2)}
                      </p>
                      <p className="text-base font-medium text-red-500">
                        {currencySymbol}
                        {account.expense_total.toFixed(2)}
                      </p>
                      <hr className="my-2 w-24 ml-auto border-gray-300" />
                      <p className="text-base font-medium">
                        {currencySymbol}
                        {account.remainder.toFixed(2)}
                      </p>
                    </div>
                  </div>
                ))}
